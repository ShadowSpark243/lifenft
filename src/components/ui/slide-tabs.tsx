...
export const SlideTabs = ({ tabs = ["Home", "Pricing", "Features", "Docs", "Blog"], onTabChange, defaultIndex = 0 }) => {
    ...
    // Sync selected tab with defaultIndex when it changes (e.g. route changes)
    useEffect(() => {
    ...
    return (
        <ul
            ...
            className="relative mx-auto flex w-fit rounded-full border-2 border-slate-600 bg-slate-800/80 p-1 backdrop-blur-sm"
        >
            {tabs.map((tab, i) => (
                <Tab
                    key={tab}
